...
// 運算時，*運算子會將字串8會轉型為數字再進行運算，故運算結果為 3* '8'= 24
// 由四則運算先計算後比較的特性，此題為 24 > 21 答案為 true
//--- 練習結束
...
console.log("-----題目三-----");
let isVip = true;
if (isVip === BobIsVip || BobPrice > giftPriceRule) {
  giftNum -= 1;
  console.log("客戶您好，您有符合贈品資格");
} else {
  console.log("客戶您好，您沒有符合贈品資格");
...
//--- 練習區
console.log("-----題目四-----");
let coachIncome = 240000;
function calcBonus(coachIncome) {
  let baseBonus = 6000;
  switch (true) {
    case coachIncome < 100000:
      return baseBonus + coachIncome * 0.1;

    case 100000 < coachIncome && coachIncome < 300000:
      return baseBonus + coachIncome * 0.15;
...
let coachBonus = calcBonus(coachIncome);
console.log(`小明總共需支付 $${coachBonus} 獎金`);
//--- 練習結束
...
function randomChoices() {
  const rockPaperScissors = Math.floor(Math.random() * choices.length);
  return choices[rockPaperScissors];
}

//判斷輸贏
function winOrLose(playerA, playerB) {
  if (playerA === playerB) {
    return "平手";
  } else if (
    (playerA === choices[2] && playerB === choices[1]) ||
    (playerA === choices[0] && playerB === choices[2]) ||
    (playerA === choices[1] && playerB === choices[0])
  ) {
    return "playerA 勝利";
  } else {
    return "playerB 勝利";
...
//剪刀石頭布遊戲開始
function playGame() {
  let playerA = randomChoices();
  let playerB = randomChoices();
  let result = winOrLose(playerA, playerB);
  ...
  coaches: [
    {
      ...
      expertise: ["力量訓練", "減重課程"],
      courses: [
        { name: "個人訓練課程", fee: 2000, duration: 60, isAvailable: true },
        { name: "團體訓練課程", fee: 1500, duration: 90, isAvailable: false },
      ],
      background:
        "王教練擁有 5 王教練擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員。",
      canAcceptNewStudents: true,
    },
    {
      coachName: "李教練",
      expertise: ["瑜伽", "體態雕塑"],
      ...
      background:
        "李教練是一位瑜伽大師，擁有 10 李教練是一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員。",
      canAcceptNewStudents: false,
    },
  ],
};
console.log(gym);
